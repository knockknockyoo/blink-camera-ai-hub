...
The downloader, AI analyzer pool, and Telegram retry notifier run independently. A completed download is atomically added to `data/raw` and submitted to the AI pool immediately while later downloads continue. Two videos are analyzed concurrently by default, each with an independent model instance. Each relevant result is sent to Telegram as soon as that video finishes; failed deliveries remain queued across container restarts. Temporary `.part` files are never analyzed.
